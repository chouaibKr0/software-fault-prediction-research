from src.hpo.base_optimizer import BaseOptimizer
import math
import numpy as np
import random
from src.hpo import sso_decoder
from src.evaluation.cross_validation import evaluate_model_cv_mean
...
class SalpSwarmOptimizer(BaseOptimizer):
    ...
    def objective_function(self, config, objective_metric, X, y):
        ...
        return objective_fn
    def _display_iteration_compact(self, iteration, fitness):
        """Compact iteration display."""
        print(f"Iteration {iteration:3d}: Best = {fitness:.6f}")
        ...
        for t in range(self.max_iter):
            ...
            for i in range(self.num_salps):
                ...
                if fitness < self.food_fitness:
                    self.food_fitness = fitness
                    self.food_position = self.positions[i].copy()
            self._display_iteration_compact(t + 1, self.food_fitness)
